...
class Featherlite_Masthead_Control_Customizer_Control extends WP_Customize_Control {
	...
	public function render_content() {
		if ( ! is_array( $this->choices ) || ! count( $this->choices ) ) {
			printf( __( 'No masthead components found.', 'featherlite' ) );
		} else {
			$components         = $this->choices;
			$order              = $this->value();
			$disabled			= $this->_get_masthead_disabled_components( $this->value(), $components );
			?>
			<label>
				<?php
					if ( ! empty( $this->label ) ) : ?>
					...
					<?php endif;
				?>
				<ul class="masthead-control">
					<?php $components = $this->_reorder_masthead_components( $components, $order ); ?>
					<?php foreach ( $components as $id => $title ) : ?>
						<?php
							 ...
							 */
							$title = apply_filters( 'masthead_control_title', $title, $id );
	 ...
	 */
	private function _reorder_masthead_components ( $components, $order ) {
		$order_entries = array();
		if ( '' != $order ) {
		...
		if ( 0 < count( $order_entries ) ) {
			...
			foreach ( $order_entries as $k => $v ) {
				if ( $this->_is_masthead_component_disabled( $v ) ) {
	 ...
	 */
	private function _is_masthead_component_disabled ( $component ) {
	 ...
	 */
	private function _get_masthead_disabled_components ( $saved_components, $all_components ) {
		$disabled = array();
		if ( '' != $saved_components ) {
