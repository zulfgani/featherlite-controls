...
class Featherlite_Controls_Customizer {
	/**
	...
	public function customizer_setup ( $wp_customize ) {
		...
		$wp_customize->add_section( 'homepage_control', array(
			'title'          => __( 'Homepage Controls', 'homepage-control' ),
			'priority'       => 5,
			'panel' => 'featherlite_homepage_panel',
		) );
		...
		require_once( 'class-featherlite-controls-customizer-control.php' );

		$theme = wp_get_theme();

		$wp_customize->add_control( new Featherlite_Controls_Customizer_Control( $wp_customize, 'homepage_control', array(
			'description'       => sprintf( __( 'Enable/Disable/Re-order the homepage components in %s.', 'homepage-control' ), $theme->__get( 'Name' ) ),
	...
	public function enqueue_scripts() {
		wp_enqueue_script( Featherlite_Controls()->token . '-sortables', esc_url( Featherlite_Controls()->plugin_url . 'assets/js/sortables.js' ), array( 'jquery', 'jquery-ui-sortable' ), Featherlite_Controls()->version );
	}
	...
	public function enqueue_styles() {
		wp_enqueue_style( Featherlite_Controls()->token . '-customizer',  esc_url( Featherlite_Controls()->plugin_url . 'assets/css/settings.css' ), '', Featherlite_Controls()->version );
	}
	...
	*/
	...
	/**
	 * Retrive the functions hooked on to the "featherlite_homepage" hook.
	 * @access  private
	 * @since   1.0.0
	...
	private function _get_hooked_functions () {
		...
		if ( isset( $wp_filter[Featherlite_Controls()->hook] ) && 0 < iterator_count( $wp_filter[Featherlite_Controls()->hook] ) ) {
			foreach ( $wp_filter[Featherlite_Controls()->hook] as $k => $v ) {
				if ( is_array( $v ) ) {
					foreach ( $v as $i => $j ) {
	...
	private function _maybe_format_title ( $key ) {
		$prefix = (string)apply_filters( 'hompage_control_prefix', 'featherlite_display_' );
		$title = $key;
...
new Featherlite_Controls_Customizer();
