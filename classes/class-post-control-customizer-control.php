...
class Featherlite_Post_Control_Customizer_Control extends WP_Customize_Control {
	...
	public function render_content() {
		if ( ! is_array( $this->choices ) || ! count( $this->choices ) ) {
			printf( __( 'No post components found.', 'featherlite-controls' ) );
		} else {
			$components         = $this->choices;
			$order              = $this->value();
			$disabled			= $this->_get_post_disabled_components( $this->value(), $components );
			?>
			<label>
				<?php
					if ( ! empty( $this->label ) ) : ?>
				...
				<ul class="post-control">
					<?php $components = $this->_reorder_post_components( $components, $order ); ?>
					<?php foreach ( $components as $id => $title ) : ?>
						<?php
							 ...
							 */
							$title = apply_filters( 'post_control_title', $title, $id );
	 ...
	 */
	private function _reorder_post_components ( $components, $order ) {
		$order_entries = array();
		if ( '' != $order ) {
		...
		if ( 0 < count( $order_entries ) ) {
			...
			foreach ( $order_entries as $k => $v ) {
				if ( $this->_is_post_component_disabled( $v ) ) {
	 ...
	 */
	private function _is_post_component_disabled ( $component ) {
	 ...
	 */
	private function _get_post_disabled_components ( $saved_components, $all_components ) {
		$disabled = array();
		if ( '' != $saved_components ) {
